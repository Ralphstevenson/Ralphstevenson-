...
// ==========================================
// VII. SISTÈM CHAT AVANSE (KLIYAN)
// ==========================================

import { ref, push, onValue, update, serverTimestamp, set, onDisconnect, get } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-database.js";

let isChatPageActive = false;
const notifSound = document.getElementById('notif-sound');

/**
 * 1. JERYON PREZANS (Online/Offline)
 * Sa ap jere pwen vèt/wouj la pou Admin nan ak Kliyan an
 */
window.setupPresence = (uid) => {
    ...
    onValue(connectedRef, (snap) => {
        if (snap.val() === true) {
            ...
            // Lè kliyan an konekte
            set(myStatusRef, { state: "online", last_changed: serverTimestamp() });
        }
    });
    
    // Koute si Admin nan Online pou afiche pwen an vèt
    onValue(ref(db, `status/admin`), (snap) => {
        ...
        if (dot && txt) {
            ...
            txt.innerText = status === "online" ? "An liy" : "De-konekte";
        }
    });
};

/**
 * 2. VOYE MESAJ (Tèks oswa Foto)
 */
window.sendMessage = async () => {
    ...
    try {
        const chatRef = push(ref(db, `chats/${auth.currentUser.uid}/messages`));
        await set(chatRef, {
            sender: "client",
            text: msg,
            type: "text",
            status: "sent", // 'sent' = yon sèl koup pal
            timestamp: serverTimestamp()
        });
        input.value = ""; // Netwaye input la
    } catch (e) { 
        console.error("Erè voye mesaj:", e); 
    }
};
...
window.uploadImage = (input) => {
    ...
    reader.onload = async (e) => {
        ...
        await set(chatRef, {
            sender: "client",
            url: e.target.result, // Done imaj la
            type: "image",
            status: "sent",
            timestamp: serverTimestamp()
        });
...
window.listenToMessages = (uid) => {
    ...
    onValue(chatRef, (snapshot) => {
        ...
        const badge = document.getElementById('chat-badge'); // Si ou gen yon badj nan nav
        let unreadCount = 0;
        ...
        if (data) {
            Object.keys(data).forEach(key => {
                ...
                // Konte mesaj admin ki poko li
                if (m.sender === "admin" && m.status !== "read") {
                    unreadCount++;
                }

                // Desen Ti Koup (Double Check)
                let checkHtml = "";
                if (isClient) {
                    const icon = m.status === "read" ? "fa-check-double text-blue" : "fa-check text-gray";
                    checkHtml = `<i class="fa-solid ${icon} check-mark"></i>`;
                }

                // Kontni (Imaj oswa Tèks)
                const content = m.type === "image" 
                    ? `<img src="${m.url}" class="chat-img" onclick="window.open('${m.url}')">` 
                    : `<span>${m.text}</span>`;

                messagesDiv.innerHTML += `
                    <div class="${isClient ? 'msg-user' : 'msg-admin'}">
                        ${content}
                        ${checkHtml}
                    </div>`;
            });

            // Lojik Son ak Notifikasyon
            if (unreadCount > 0 && !isChatPageActive) {
                if(badge) {
            ...
            // Si paj chat la ouvè, make mesaj yo kòm "li" (read)
            if (isChatPageActive && unreadCount > 0) {
                markMessagesAsRead(uid, data);
            }
            
            // Scroll anba nèt otomatikman
...
/**
 * 4. MAKE MESAJ YO KÒM "LI"
 */
const markMessagesAsRead = (uid, messages) => {
    Object.keys(messages).forEach(key => {
        if (messages[key].sender === "admin" && messages[key].status !== "read") {
            update(ref(db, `chats/${uid}/messages/${key}`), { status: "read" });
        }
    });
...
/**
 * 5. ENTÈGASYON AK NAVIGASYON (showPage)
 * MODIFYE fonksyon showPage ou a pou ajoute liy sa yo
 */
const originalShowPage = window.showPage;
window.showPage = (id, el) => {
    // Rele fonksyon orijinal la si li egziste
    if (typeof originalShowPage === 'function') {
        originalShowPage(id, el);
    } else {
        // Si se kòd m te ba ou a, li fè sa:
        document.querySelectorAll('main section').forEach(s => s.classList.add('hidden'));
        const target = document.getElementById(id);
        if(target) target.classList.remove('hidden');
        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
    ...
    // Tcheke si se paj chat la ki ouvè
    isChatPageActive = (id === 'chat-container');

    if (isChatPageActive && auth.currentUser) {
        // Lè paj la ouvè, nou make tout mesaj kòm "li"
        get(ref(db, `chats/${auth.currentUser.uid}/messages`)).then((snap) => {
            if(snap.exists()) markMessagesAsRead(auth.currentUser.uid, snap.val());
        });
        // Scroll anba
        const messagesDiv = document.getElementById('chat-messages');
