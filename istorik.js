/* ============================================================
   JS ISTORIK PRO - ECHANJ PLUS V4.5 (FIXED SYNC)
   ============================================================ */
...
window.initIstorik = (uid) => {
    ...
    // 1. MONTRE SKELETON (LOADING)
    const toutList = document.getElementById('list-tout');
    if (toutList) toutList.innerHTML = `<div class="skeleton-loader">Loading...</div>`;
    ...
    onValue(transRef, (snap) => {
        ...
        myTrans.forEach(t => {
            // Nou tcheke si se amount_sent (Echanj) oswa amount (Retrè)
...
function createCardHTML(t, montan) {
    let color = t.status === "Validé" ? "#36b37e" : (t.status === "En attente" ? "#ffab00" : "#ff5630");
    let icon = t.type === "Echanj" ? "fa-rotate" : "fa-money-bill-transfer";

    return `
        <div class="transaction-item animated fadeIn" style="border-left: 4px solid ${color}; background:#fff; padding:15px; border-radius:12px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div>
                    ...
                    <div style="font-size:10px; color:#64748b;">${new Date(t.timestamp).toLocaleDateString()}</div>
                </div>
            </div>
