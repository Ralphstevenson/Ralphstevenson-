...
export function initIstorik(uid) {
    ...
    // Koute dirèkteman branch tranzaksyon itilizatè a pou vitès ak sekirite
    const userTransRef = ref(db, `users/${uid}/transactions`);
    
    onValue(userTransRef, (snap) => {
        ...
        // Tcheke si gen tab ki vid apre distribisyon an pou n mete mesaj "Vid" la
        sections.forEach(s => {
            const el = document.getElementById(`list-${s}`);
